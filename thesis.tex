...
\thesissetup{
    ...
    abstract    = {%
      This thesis aims to solve the degrading state of the web application of the Online Seminar of Informatics (KSI). The current web application is built upon currently unsupported web framework, which cannot be easily updated. To resolve this issue, this thesis creates a new implementation of said web application, which will use such a technology that is expected to be maintainable in the long term. First, it discusses which technology is best suited for given use case and then presents the solution, together with changes made to the KSI backend. As as a result, the new web application is shown, together with two active deployments, one of which is used as a production environment for a different activity. This is possible due to high focus on easy modularity throughout the developement process.
    },
    thanks      = {%
       I want to thank my advisor, RNDr. Vladimír Štill, Ph.D., for his invaluable insights and advice throughout this thesis. Likewise, I would like to thank the entire KSI organizers team for frequent feedback and words of reinforcement during the development process. Finally, I would like to express my gratitude to my girlfriend and family for their unending support.
    },
    bib         = thesis.bib,
...
As a part of this thesis, some related modifications are also made to the server part of the KSI infrastructure.

The first chapter describes what requirements are laid upon the KSI web application. The second chapter depicts the current KSI infrastructure, both frontend and backend. The third chapter discusses possible approaches to the development of new web application. Chapters 4 and 5 presents the newly created projects for manipulation with backend and the new web application respectively. The final web application, which is a primary goal of this thesis,  is already [publicly deployed](https://ksi.fi.muni.cz/novy-web/).
...
\mdchap{Components of the KSI web application}

Given that a web frontend for KSI already exists and this thesis is aimed to be a revitalization of the KSI frontend, it is expected for the new web frontend to implement the same features as can be found in the current one. These features are discussed in separate sub-chapters.

#### Learning task

The KSI participants will spend most of their time of pages containing tasks. Every task does have an assigment text, in which the subject of given tasks is explained first and then examined in so called modules. Modules can take following forms:

- text input question
- programming task
- single choice question
- multiple choice question
- correct sorting task
- uploading a file

Every module has a certain number of points which are awarded to the participant based on his solutions and a single task can have multiple modules. Moreover, every task has its own discussion thread in which participants can submit their questions and also a original solution which is made available to the participant after successful submission of the task.

All tasks are grouped into tic units -- waves -- which gradually explain a single subject to the participant. Because often a task requires knowledge of content from another task, the tasks can be ordered in such way that to unlock one task a participant has to solve all previous tasks. This prerequisites are best represented as an acyclic oriented graph, which has to be visible to the participants for easier orientation.

#### Discussion

Even though every tasks has its own discussion attached, some questions that participants may have are not related to a single task. For this reason a separate general discussion is required, where anyone can create a new discussion thread.

#### Results

The participants are sorted based upon their earned number of points on a results page. The KSI differentiates between participants from high school and others. If the participant gains at least 60 \% of all available points from all tasks, he is awarded the title of a successful participant. Such participant needs to have different color in the results table.

#### General features

Because KSI is an annual competition, the frontend needs to present option for selecting which year should be displayed to the user. The web application should also support logging in for already registered users and registering new users. Every registered user should have a public profile with part of the information visible only after logging in.

A pages with information about the online seminar itself and about news in the current year are also to be included. Furthermore, every user can be granted an achievement trophy for completing a special assignment. Every such trophy needs a page with its description and icon.
...
For its supposed use case, which is different from creating complex applications consisting of custom elements, the jQuery fully lacks advanced features like abstract component creation and styles encapsulation.

### React.js

React.js~[@reactjs] is a web framework that introduces its own syntax called JSX~[@reactjs-main-concepts]. JSX is a syntax extension of JavaScript, which allows writing HTML template together with JavaScript logic:
...
Angular module can be compared to a C header file -- every Angular component needs to be declared inside an Angular module. After building an Angular application, every module is located in its own JavaScript files. These files then have to be downloaded to allow the part of the application that uses them to work properly. This feature is best exampled when used together with so called "lazy-loading"~[@angular-lazy-modules]. By default, all modules are loaded as soon as they are available to the connected client for download. On the other hand, when the module is marked for lazy-loading, it is downloaded only when requested by the client. This approach saves bandwidth, as the client does not always need all components of the applications -- for example user that does only want to read the news does not need to download all task module types.
...
From this code example, it is evident, that the endpoint is expecting a JSON dictionary with a single key `post`, which contains additional dictionary with keys `author` and `body`. This information can be transformed into TypeScript interfaces in a following way:
...
From this information, the TypeScript OpenAPI is able to generate the `swagger.json` endpoint description, which can be visualized by Swagger UI, as seen in figure \autoref{fig:swagger-ui}.
...
Because the definition of the API is stored within a separate project, this could in the future lead to a de-synchronization between the definition of the API and the backend code itself. Though, this threat would be present in some form unless the `swagger.json` can be generated directly from the backend code. If no technology like OpenAPI was used, then every time a backend endpoint would change, all frontend usages of such endpoint would need to be manually reviewed. The separate project is a compromise, which minimizes the risk of de-synchronization by using a simple to use syntax for the backend developers and easy changes implementation by regenerating the code from the Swagger definition for the frontend developers.
...
The data of the backend server, together with the SQLite database file, are kept externally and are only bind mounted to the container, so that rebuilding the docker image does not erase them. Only when the entrypoint of the container is executed the data is mounted into appropriate location. To prevent insufficient permission when accessing to the data by the unprivileged `ksi` user inside the container, the real owner is masked by using `bindfs` tool. The `bindfs` is also used to bind-mount `/etc/` to `/opt/etc` to bypass higher permission requirements for the Docker container. 

Because the container is running in a privileged mode with shared `/dev/fuse` for the `bindfs` tool, it is currently undesirable to execute production server inside the container, as it may pose a security risk. On the other hand, the container is fully suitable for usage during the development of the frontend application. Furthermore, the whole docker container can be packaged inside a `.tar` file and then transferred to another machine, making the process of creating another backend instance faster. This feature was used when deploying a temporary [testing server](https://ksi.ahlava.cz/api/years), which is available for all KSI organizers and is running inside the docker container.

\end{markdown*}

\mdchap{Final web application}

This chapter describes the implementation of the new web application frontend itself. In the first part this thesis analyzes the application architecture and code structure while highlighting specific decisions made during the development, while the second part is focused on design of user interface presented directly to the user.

Next, the modularity of the new web frontend is presented on the example of second deployed instance and the approach to presenting historic KSI tasks on the new web application is described. Finally, the new frontend is evaluated based on feedback received from its users.
...
The project's root `util` subdirectory contains shell scripts for automatizing tasks. The PWA (for explanation see \autoref{chap:faster}) requires icon to be of exact size, while the new web applications uses an SVG icon, the `gen-icons.sh` script takes the main SVG icon and converts into multiple icons with fixed resolution for the PWA. The `gen-api.sh` script automatically downloads the correct version of `swagger-codegen` tool (\autoref{chap:swagger-codegen}) and executes it to generate API code inside the `src/api` directory. It also attempts to fix the code that was generated in a faulty way -- for example by correcting the type of recursive array or by unescaping HTML text (`'&lt;'` is replaced for `'<'`; `'&gt;'` with `'>'`) and modifying incorrectly typed function for downloading file from the backend.
...
Throughout the development process, the KSI organizer team members have submitted multiple enhancement ideas and discovered bugs, which were either already incorporated or require further discussion. To evaluate the final web application a questinare about experience with the application was sent to the KSI participants. The responses to the questinare were positive, most notably with regard to the mobile version of the web application, which is now fully usable with all features on desktop available.

At the final meeting of the KSI organizers teams was decided that the new web application will replace the current one with the start of the KSI year 2022/2023 in September. In the meantime, the new web application is already being used in a new online activity from the Faculty of Informatics Masaryk Univerzity, which is set to launch on the 25th May 2022. For this new activity, the web application's color schema and internationalization texts were modified to meet activity's requirements, underlining the goal of easy modifiability.

In the future, the new web application is expected to be further actively developed. This next development is going to reflect breaking changes that are to be made to the KSI backend, which will make the current web frontend unusable and is scheduled on the summer of 2022. These changes could not be performed before, because it was deemed unsafe to perform breaking changes while the KSI year was in progress. Furthermore, the historic KSI tasks, which are now available on the public KSI testing server, are to be merged with the production database after additional steps like creating icons for them to better fit in with the rest of the tasks. All these tasks also require heavy cooperation of multiple people, which is why it was scheduled outside the university semester.

Because all projects created as part of this thesis are published under [GNU General Public License v3.0](https://choosealicense.com/licenses/gpl-3.0/#), they may be freely used by anyone.
...
\mdstart
The implementation of this thesis is submitted to the archive as a zip archive, which contains all three projects as root folders -- the web application frontend, the Swagger backend proxy project and the project for conversion of \LaTeX{} tasks to the SQL statement. The archive, and every contained project's directory, contains a `README.md` file, which describes the structure and requirements for starting the project.
